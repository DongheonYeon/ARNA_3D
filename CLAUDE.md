...
```python
from core import core_smooth

# gRPC 등 백엔드에서 호출
output_path = core_smooth(
    input_path="./data/case_S004/mask/segment_A.nii.gz",
    output_path="./data/case_S004/3d/obj_A.glb"
)
```
...
ARNA-3D/                     # 프로젝트 루트
├── __init__.py              # 패키지 초기화
├── core.py                  # 백엔드 통합용 래퍼 (core_smooth)
├── pipeline.py              # 핵심 파이프라인 (Pipeline 클래스, run_pipeline)
│
├── config/                  # 설정 및 상수
...
│   ├── nifti.py             # NIfTI 읽기/쓰기, sitk_to_volume
...
# 파이프라인 설정
PipelineSettings(input_path, output_path, debug)
```
